...
setInterval(async () => {  
  ...
  finishedPenals.forEach(async (x) => {
    ...
    if (x.type === "CHAT-MUTE") {
      ...
      client.channels.cache.find(x => x.name == "mute_log").send({ embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`${member.toString()} adlı Kullanıcının **Chat Mute** süresi doldu`)]});
    }
    if (x.type === "TEMP-JAIL") {
      x.active = false;
      await x.save();
    ...
    if (x.type === "VOICE-MUTE") {
      ...
      client.channels.cache.find(x => x.name == "vmute_log").send({ embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`${member.toString()} adlı Kullanıcının **Ses Mute** süresi doldu`)]});
    }
  });
