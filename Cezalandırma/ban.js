...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.react(green)
    if (settings.dmMessages) user.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle banlandınız!`}).catch(() => {});
...
${kirmiziok} Ban Sebebi: \`${reason}\`
      ...
      client.channels.cache.find(x => x.name == "ban_log").send({ embeds: [embed.setDescription(`
      ${Cezaa} Banlanan Üye: **${member ? member.toString() : user.username}** \`${user.id}\`
      ${Revuu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
      ${kirmiziok} Ban Sebebi: \`${reason}\``)] });
    ...
    client.channels.cache.find(x => x.name == "cezapuan_log").send({ content:`${member} üyesi ban cezası alarak toplam \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanına\` ulaştı!`});
