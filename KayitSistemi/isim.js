...
run: async (client, message, args, prefix) => { 
  ...
  let ayar = await serverSettings.findOne({
    ...
    {
    message.react(red)
    message.reply({ content:`\`.isim <@ramal/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}isim <@ramal/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    if(!yaş) 
...
.setFooter({ text: `Üyesinin önceki isimlerine <!isimler @ramal/ID> komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.` });
